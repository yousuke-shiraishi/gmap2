...
function initMap() {
  ...
  for (var i = 0; i < n; i++) {
    ...
    markerEvent(i, largeInfowindow, m);
  ...
  function markerEvent(i, largeInfowindow, marker) {
    marker.addListener('click', function() { // マーカーをクリックしたとき
      if (largeInfowindow[i].marker != marker) {
        largeInfowindow[i].marker = marker;
        largeInfowindow[i].setContent('<div><p>' + marker.title + '</p></div>' + '<div><p>' + largeInfowindow[i].content + '</p></div>');
        ...
        largeInfowindow[i].addListener('closeclick', function() {
          ...
          $.ajax({
            ...
            function() {
              alert('他の人のマーカー情報は消せません。又データーベースへの反映はマーカー作成日から4日後です。');
            });
