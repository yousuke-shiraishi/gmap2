...
function markerEvent(i, largeInfowindow, marker, address,map) {
  marker.addListener('click', function() { // マーカーをクリックしたと
    if (largeInfowindow[i].marker != marker) {
      ...
      largeInfowindow[i].setContent('<img class="picture" src=' + address.image["url"] + ' width="100%" height="100%" alt=""/>' + '<div><p>' + marker.title + '</p></div>' + '<div><p>' + largeInfowindow[i].content + '</p></div>');
