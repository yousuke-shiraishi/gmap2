...
function markerEvent(i, largeInfowindow, marker, address,map) {
  marker.addListener('click', function() { // マーカーをクリックしたと
    if (largeInfowindow[i].marker != marker) {
      ...
      largeInfowindow[i].setContent('<a href="'+address.image["url"]+'"target="_blank"data-lightbox="image-' + marker.id + '"> <img class="picture" src=' + address.image["url"] + ' width="50%" height="50%" alt=""/></a>' + '<div><p>' + marker.title + '</p></div>' +
      '<div><p>' + largeInfowindow[i].content + '</p></div>' + '<div><input type="button" value="マーカーを削除する" id="btn'+ marker.id +'"/></div>');
      largeInfowindow[i].open(map, marker); // 吹き出しの表示
      const target = document.getElementById('btn' + marker.id + '');
      target.addEventListener('click',function(e){
      //  console.log("marker"+marker.id);
    //largeInfowindow[i].addListener('clickclose', function(){
